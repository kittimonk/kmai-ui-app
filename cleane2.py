...
def build_edh_logic(edh_df, source_malcode=None, source_table=None):
    """Build SQL-like logic from EDH reference sheet."""
    logic_parts = []
    for _, row in edh_df.iterrows():
        src = row.get("source_field", "")
        val = row.get("source_value", "")
        tgt = row.get("target_field", "")
        tgt_val = row.get("target_value", "")
        cond = f"WHEN {src} = '{val}' THEN {tgt} = '{tgt_val}'"
        logic_parts.append(cond)
    if logic_parts:
        return "CASE " + " ".join(logic_parts) + " END"
    return ""
    ...
    for _, row in mapping_df.iterrows():
        ...
        if "EDH derived" in str(row.get("source table", "")) or \
           ...
           "EDH derived" in str(row.get("transformation rule auto populate (free form text)", "")):
            edh_logic = build_edh_logic(edh_df)
        edh_logic_list.append(edh_logic)
